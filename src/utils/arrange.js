...
export const MAX_USERS_ON_TABLE = 6;
export const TABLES = TableConfig.tables || [];
 ...
 */
export function usersOnTable(users, tableId) {
 ...
 */
export function usersToTables(users) {
...
function getFreeTable() {
  const sortedTables = [...TABLES].sort((a, b) => {
    const A = usersOnTable(a).length;
    const B = usersOnTable(b).length;
  ...
  const mostFreeTableSeatsCount = usersOnTable(sortedTables[0]);
